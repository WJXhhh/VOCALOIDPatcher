...
public sealed class VocaloidTools
{
    ...
    [Description("Query project structure, tracks, parts, notes, tempo, time signatures, parameters, or selection with revision-bound pagination.")]
    public Task<McpBridgeResult> QueryProject(
        ...
        CancellationToken cancellationToken = default)
        => _gateway.InvokeAsync(server, "v6_query_project", instance_id, new { kind, page_size, page_token, filter }, cancellationToken);
    ...
    [Description("Create, open, save, save-as, or export MIDI. File access is enforced by the V6-side allowlist and confirmation policy.")]
